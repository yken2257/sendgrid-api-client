...
export function DateTimeForm({ filter, operator, value, onChange }) {
  ...
  const defaultTime = operator === '<' || operator === '>=' ? undefined : '09:00:00';
  ...
  useEffect(
    () => {
      const dateAndTimeValue = dateValue + ' ' + (timeValue || '00:00:00');
...
function parseDateTimeFilter(filter) {
  const regexDate = /^(\d\d\d\d(-|\/\d\d)?(-|\/\d\d)?)( \d\d(:\d\d)?(:\d\d)?)?/;
  const dateTime = filter.match(regexDate)?.[0] || '';

  let [dateValue, timeValue = ''] = dateTime.split(' ');
...
export function matchDateTime(operator, itemValue, tokenValue) {
  const itemTime = new Date(itemValue).getTime();
  const tokenTime = new Date(tokenValue).getTime();
  if (operator == ">=") {
    return itemTime >= tokenTime
  } else if (operator == "<=") {
    return itemTime <= tokenTime
  }
}
